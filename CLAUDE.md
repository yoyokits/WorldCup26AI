...
- **Interactive calendar view:** A compact, small calendar (roughly 25% the size of a full-width card — do not let it dominate the page). Month-at-a-time display (not the full tournament span at once). Default to June 2026. Users can switch between June and July via prev/next buttons or month tabs. Each date with matches is highlighted/clickable. When a user clicks a date, the matches appear to the **right side** of the calendar (side-by-side layout on desktop; stacked on mobile). The matches panel stretches the full remaining horizontal width. Each match card is **interactive/expandable** — clicking a match expands it to show full statistics (win/draw/loss probabilities, Elo ratings, expected goals, contextual factors) just like the group page match cards. Match cards show teams, flags, kickoff times (local timezone), venue, **and the predicted score**. Uses vanilla JavaScript.
...
3. **Interactive match cards (JavaScript):** On pages that show prediction results (group pages, knockout pages, **and calendar match panels**), each match should be clickable. When a user clicks a match row/card, it expands to reveal additional statistics:
...
4. **Author attribution:** Every page footer must include the author name: **Yohanes Wahyu Nurcahyo**.

5. **Prediction method page:** `index.html` must link to `prediction_method.html` — an HTML page that explains:
...
6. **Standings table legend:** Every group standings table must include a legend below it explaining column abbreviations: P = Played, W = Won, D = Drawn, L = Lost, GF = Goals For, GA = Goals Against, GD = Goal Difference, Pts = Points.

7. **Hero overlay click-through:** The hero section uses a `::before` pseudo-element for a decorative pattern overlay. All interactive elements inside the hero (buttons, links) must have `position: relative` so they remain clickable above the overlay.
